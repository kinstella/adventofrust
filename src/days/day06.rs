...
pub fn find_unique_set(ln : usize, orig_str : String) {
    let mut vstr = orig_str.chars().map(|c| c.to_string()).collect::<Vec<String>>();
    let mut charct = 0;
    let mut items = vstr.iter().take(ln).map(|c| c.to_string()).collect::<HashSet<String>>();
    while items.len() < ln {
        vstr = vstr[1..].iter().map(|c| c.to_string()).collect();
        items = vstr.iter().take(ln).map(|c| c.to_string()).collect::<HashSet<String>>();
        charct = 1 + charct;
    }
